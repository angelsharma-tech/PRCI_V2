...
class TwoHeadInfer:
    def __init__(self, ckpt_path: str):
        """
        Load the trained two-head BERT from a checkpoint.
        Handles both full training checkpoints and raw state_dict exports.
        """
        self.ckpt_path = ckpt_path
        ckpt = torch.load(ckpt_path, map_location="cpu")
        
        # Backward Compatibility: Detect checkpoint format
        if isinstance(ckpt, dict) and "state_dict" in ckpt:
            print(f"[TwoHeadInfer] Full training checkpoint detected: {ckpt_path}")
            args = ckpt.get("args", {})
            state_dict = ckpt["state_dict"]
        else:
            print(f"[TwoHeadInfer] Raw state_dict detected: {ckpt_path}")
            args = {}
            state_dict = ckpt
